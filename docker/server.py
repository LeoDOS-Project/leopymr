...
from routing import get_direction, add_direction, get_dist_hops
...
class ISL:
    ...
    def send(self, sat, payload, files={}):
        ...
        if target_id != sat:
            ...
            log(
                f"ISL from {target_id} to {sat} direction {direction} next {next_sat}",
                context=payload)
        else:
            next_sat = sat
...
def send():
    ...
    if data["target"] == target_config["id"]:
        ...
        threading.Thread(
            target=send_data, args=(
                host, port, "send", data, filedump(
                    request.files))).start()
...
def submit():
    ...
    return json.dumps({"reducer": reducer,
                       "los": target.get_id(),
...
def completion():
    ...
    if target.is_reduce_done():
        ...
        if target.reduce_files is not None:
            result_files = []
