...
class Satellite:
    ...
    def dispatch(self, payload, files):
        ...
        log("Dispatching", sat=self, context=payload)
        if action == "collect":
            ...
            log(
                f"Setting expected map count {
                    self.expected_map_count}",
                sat=self,
                context=payload)
        else:
            log("Unknown action", sat=self, context=payload, verbosity=0)

    def collect(self, payload):
        collect_task = comp_finder.find_collect(
            payload["meta_data"]["collect_task"])
        total_collected = 0
        data = []
        files = {}
        ...
        for record in collect_task.collect(payload):
            ...
            if isinstance(record, dict) and "_COMP_FILE_" in record:
                files[record["_COMP_FILE_"]["name"]
                      ] = record["_COMP_FILE_"]["stream"]
                data.append(record["value"])
            else:
                data.append(record)
            if len(data) >= payload["meta_data"]["max_collect_records"]:
                if not skip_map:
                    self.isl.send(payload["mapper"],
                                  {"meta_data": payload["meta_data"],
                                   "action": "map",
                                   "end_collect": False,
                                   "collected_index": total_collected,
                                   "data": data,
                                   "collector": self.get_id()},
                                  files)
                else:
                    self.isl.send(
                        payload["meta_data"]["reducer"],
                        {
                            "mapped_index": self.total_mapped,
                            "meta_data": payload["meta_data"],
                            "action": "reduce",
                            "end_map": False,
                            "data": self.combine(
        ...
        if not skip_map:
            ...
            self.isl.send(payload["meta_data"]["reducer"],
                          {"mapped_index": self.total_mapped,
                           "meta_data": payload["meta_data"],
                           "action": "reduce",
                           "end_map": True,
    ...
    def combine(self, data, payload):
        combine_task = comp_finder.find_combine(
            payload["meta_data"]["combine_task"])
    ...
    def run_map(self, payload):
        ...
        if payload["end_collect"] and "data" in payload and len(
                payload["data"]) == 0:
            self.isl.send(self.reducer,
                          {"mapped_index": self.total_mapped,
                           "meta_data": payload["meta_data"],
                              "action": "reduce",
                              "end_map": True,
        ...
        for mapped_data in map_task.run_map(payload):
            ...
            if isinstance(mapped_data, dict) and "_COMP_FILE_" in mapped_data:
                files = {}
                files[mapped_data["_COMP_FILE_"]["name"]
                      ] = mapped_data["_COMP_FILE_"]["stream"]
                self.isl.send(self.reducer,
                              {"mapped_index": self.total_mapped,
                               "meta_data": payload["meta_data"],
                                  "action": "reduce",
                                  "end_map": payload["end_collect"],
    ...
    def reduce(self, payload):
        ...
        log(f"Got Data from Mapper {mapper} count {self.map_count}",
            sat=self, context=payload)
        if "data" in payload:
            self.reduced_data.append(payload)
        if self.is_map_done():
            reduce_task = comp_finder.find_reduce(
                payload["meta_data"]["reduce_task"])
            self.reduce_result = reduce_task.reduce(self.reduced_data)
            if isinstance(self.reduce_result,
                          dict) and "_COMP_FILE_" in self.reduce_result:
                files = {}
                files[self.reduce_result["_COMP_FILE_"]["name"]
                      ] = self.reduce_result["_COMP_FILE_"]["stream"]
                self.reduce_files = files
            self.reduce_done = True
            self.job_time = time.time() - payload["meta_data"]["job_start"]
            self.get_reduce_result(
                {"los": payload["meta_data"]["los"], "meta_data": payload["meta_data"]})
    ...
    def reduce_response(self, payload):
        ...
        if payload["data"]["done"]:
            ...
            self.isl.send_gateway(
                {
                    "meta_data": payload["meta_data"],
                    "job_time": payload["data"]["job_time"],
                    "result": self.reduce_result})

    def set_expected_map_count(self, expected_count):
        if self.remote_reducer is not None:
            self.isl.send(
                self.remote_reducer, {
                    "meta_data": {
                        "jobid": self.jobid}, "action": "set_map_count", "data": {
