...
model = VJEPA2ForVideoClassification.from_pretrained(
    "facebook/vjepa2-vitl-fpc16-256-ssv2").eval()
...
class VJEPACollector:
    ...
    def collect(self, payload):
        ...
        log(
            f"VJEPACollector fname {fname} id {data_id} size {file_size} start {start_frame} end {end_frame}",
            context=payload)

        if not cap.isOpened():
            log(f"VJEPACollector fname {fname} cannot open video",
                context=payload, verbosity=0)
            return
        ...
        try:
            ...
            while True:
                ...
                if not ret:
                    log(
                        f"VJEPACollector fname {fname} failed to read frame",
                        context=payload,
                        verbosity=0)
                    break
                ...
                if len(frame_buffer) == NUM_FRAMES:
                    ...
                    log(
                        f"VJEPACollector Predicted label {label}",
                        context=payload)
                    yield {label: 1}
                    frame_buffer.clear()
        finally:
